...
1) Make a mirror clone (this creates the directory you’ll work in)

# from *any* directory you like (e.g., your home folder)
...
6) Tell collaborators what to do
	•	Ask teammates to delete their local clones (safest) and re-clone:

rm -rf dwp-blog-service
git clone git@github.com:devopswithprashant/dwp-blog-service.git


	•	If they can’t delete, they must at least run the same cleanup locally before pushing, or their old history could reintroduce the secret.

⸻

Notes & tips
	•	We replaced with -Dpassword=***REMOVED*** (no quotes), exactly as you wanted.
	•	The regex rules above cover double-quoted, single-quoted, and unquoted tokens, and only target values starting with glpat-. If you also had non-glpat- passwords, uncomment the “generic catch-all” rule to nuke everything assigned to -Dpassword=....
	•	After this, move credentials into CI variables (GitHub Actions Secrets / GitLab CI variables) and reference them in your CI scripts, not in the repo.
